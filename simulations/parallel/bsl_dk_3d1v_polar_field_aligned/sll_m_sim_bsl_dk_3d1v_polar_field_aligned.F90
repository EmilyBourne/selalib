...
!==============================================================================
  ...
  subroutine run_dk4d_field_aligned_polar(sim)
    ...
    sll_real64 :: advec_x3_time, advec_x4_time, advec_x12_time, poisson_time
    ...
    ! Time cycle
    !--------------------------------------------------------------------------
    advec_x3_time = 0._F64
    advec_x4_time = 0._F64
    advec_x12_time = 0._F64
    poisson_time = 0._F64
    i_plot = 0
    do iter=1,sim%num_iterations
      ...
      select case (sim%time_case)
        ...
        case (SLL_TIME_LOOP_PREDICTOR_CORRECTOR)
          ...
          advec_x3_time = advec_x3_time + sll_f_time_elapsed_since(t0)
          call sll_s_set_time_mark(t0)
          call compute_field_dk_parx1( sim )     ! updates sim%A1_parx3, A2_parx3, A3_parx1
          call advection_x4( sim, 0.5_f64*dt )
          advec_x4_time = advec_x4_time + sll_f_time_elapsed_since(t0)
          call sll_s_set_time_mark(t0)
          call advection_x1x2( sim, 0.5_f64*dt )
          advec_x12_time = advec_x12_time + sll_f_time_elapsed_since(t0)
          call sll_s_set_time_mark(t0)
          call compute_rho_dk(sim)  
          call solve_quasi_neutral_parx1( sim )
          poisson_time = poisson_time + sll_f_time_elapsed_since(t0)
    ...
    print *, "Flux advection time : ", advec_x3_time / (3*sim%num_iterations)
    print *, "V parallel advection time : ", advec_x4_time / (3*sim%num_iterations)
    print *, "Poloidal advection time : ", advec_x12_time / (2*sim%num_iterations)
    print *, "Poisson time : ", poisson_time / (2*sim%num_iterations)
